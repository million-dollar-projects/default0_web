    ...
    <SiteChrome content={content} locale={locale} sectionPrefix={`/${locale}`} languageHrefMap={languageHrefMap}>
      ...
      <main className="mx-auto w-container py-16 sm:py-20">
        <section className="rounded-xxl border border-line bg-surface p-8 sm:p-12">
          <p className="inline-flex rounded-full border border-line bg-page px-4 py-2 text-xs text-muted">{copy.badge}</p>
          <h1 className="mt-5 max-w-4xl text-4xl font-semibold tracking-tight text-text sm:text-5xl">{copy.heroTitle}</h1>
          <p className="mt-6 max-w-3xl text-lg leading-relaxed text-muted">{copy.heroDescription}</p>
        ...
        <section className="py-12">
          <div className="grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
            <article className="rounded-xxl border border-line bg-surface p-7">
              <h2 className="text-2xl font-semibold tracking-tight">{copy.problemTitle}</h2>
              <p className="mt-4 leading-relaxed text-muted">{copy.problemBody}</p>
              <p className="mt-4 leading-relaxed text-muted">
                {copy.problemLinkLead}
                <Link href={`/${locale}/blog/mac-3-auto-mute-rules-for-remote-meetings`} className="text-brand underline-offset-4 hover:underline">
            ...
            <aside className="rounded-xxl border border-line bg-gradient-to-br from-brand-soft/20 via-surface to-accent/20 p-7">
              <h2 className="text-2xl font-semibold tracking-tight">{copy.clarificationTitle}</h2>
              <p className="mt-4 leading-relaxed text-muted">{copy.clarificationBody}</p>
            </aside>
          </div>
        </section>

        <section className="py-4">
          <h2 className="text-3xl font-semibold tracking-tight">{copy.triggersTitle}</h2>
          <div className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {copy.triggerCards.map((card) => (
              <article key={card.title} className="rounded-xl2 border border-line bg-surface p-6">
                ...
                <Link href={`/${locale}/blog/${card.slug}`} className="mt-4 inline-flex text-sm font-semibold text-brand underline-offset-4 hover:underline">
                  {copy.triggerCta}
                </Link>
              </article>
            ))}
        ...
        <section className="py-12">
          <div className="rounded-xxl border border-line bg-surface p-8 sm:p-10">
            <h2 className="text-3xl font-semibold tracking-tight">{copy.comparisonTitle}</h2>
        ...
        <section className="py-4">
          <div className="rounded-xxl border border-line bg-surface p-8 sm:p-10">
            <div className="max-w-3xl">
              <h2 className="text-3xl font-semibold tracking-tight">{nextStepCopy.title}</h2>
              <p className="mt-4 text-muted">{nextStepCopy.description}</p>
            </div>
            <div className="mt-8 grid gap-4 lg:grid-cols-3">
              {nextStepCopy.cards.map((card) => (
                <article key={card.slug} className="rounded-xl2 border border-line bg-page/60 p-6">
                  <h3 className="text-xl font-semibold">{card.title}</h3>
                  <p className="mt-3 text-sm leading-relaxed text-muted">{card.body}</p>
                  <Link href={`/${locale}/blog/${card.slug}`} className="mt-4 inline-flex text-sm font-semibold text-brand underline-offset-4 hover:underline">
                    {nextStepCopy.cta}
        ...
        <section className="py-12">
          <div className="rounded-xxl border border-brand/20 bg-gradient-to-br from-[#4f89d9] via-[#5b93dd] to-[#4a84d3] p-9 text-white sm:p-12">
            ...
            <p className="mt-4 max-w-3xl text-white/85">{copy.ctaDescription}</p>
            <div className="mt-8 flex flex-wrap gap-3">
              <Link
                href={DOWNLOAD_URL}
